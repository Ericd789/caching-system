...
class CacheList:
    ...
    def __contains__(self, cid):
        ...
        current = self.head 
        prev = None
        #Handles nodelist that only have one item 
        if current is not None and current.value.cid == cid:
            return True 
...
class Cache:
    ...
    def insert(self, content, evictionPolicy):
        # YOUR CODE STARTS HERE
        Cachelevel = hash(content)
        return self.hierarchy[Cachelevel].put(content,evictionPolicy)
    ...
    def __getitem__(self, content):
        # YOUR CODE STARTS HERE
        Cachelevel = hash(content)
        if content.cid in self.hierarchy[Cachelevel]:
            return content 
        else:
            return "Cache miss!"
    ...
    def updateContent(self, content):
        # YOUR CODE STARTS HERE
        Cachelevel = hash(content)
        return self.hierarchy[Cachelevel].update(content.cid,content)
